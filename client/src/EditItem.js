...
export function EditItem(props) {
    ...
    async function submitHandler(input){
        ...
        const status = await RequestService.editItem(id, to, from, desc, loc, weight)
        if(status === 0){
            setShowAlert(true);
        ...
        <Stack>
            ...
            <FormControl>
              <InputLabel htmlFor = "id" class = "normalText">id</InputLabel>
                <Input autoComplete="off" disabled = {isLoading} id = "id" value = {id} onChange = {(e) => setID(parseInt(e.target.value))} />
            </FormControl>
            <FormControl>
            ...
            <Stack direction = 'row' justifyContent="space-evenly">
                <Button disabled = {isLoading} class = "redText" type = "submit" >Edit</Button>
                <CustomButton disabled = {isLoading} onClick = {(e)=>{props.setShowEditItem(false)}}>Close</CustomButton>
            </Stack>
